alter view top_5_posts rename to top_5_mood;

create or replace view top_5_mood as (

with like_counts as (
select ep.id, pl.mood, count(i) as like_count, ep.created_at from exposed_posts ep
left join playlists pl on ep.playlist_id = pl.id
left join interactions i on ep.post_id = i.post_id
group by ep.id, pl.mood)

select ep.* FROM
    (select *, row_number() over(partition by mood order by created_at desc) rn
    from like_counts lc
    where lc.like_count = (
